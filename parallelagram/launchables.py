...
class Lambdable:
    ...
    def try_getting_response(self):
        ...
        if remote_response is not None:
            ...
            if (
                remote_response.get("status") == "in progress"
                and remote_response.get("response") == "N/A"
            ):
                LOGGER.info(
                ...
                if "s3_response" in remote_response.get(
                    "response", {}
                ) and remote_response.get("response").get("s3_response"):
    ...
    def check_for_error(self):
        if self._response:
            ...
            if (
                response == "N/A"
                or status.lower() == "failed"
                or "UnhandledException" in response
            ):
                self.error = True
